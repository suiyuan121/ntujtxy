...
<body>
	...
	<div class="tab">
		<form name="addQuestionForm" id="addQuestionForm" method="post"
			action="daliyQuestionAdd.htm">
			<ul>
				<li>
					<span>
						<i class="must">*</i>
					...
					<a class="emng-select" id="type">
						...
						<select onchange="change($(this))" name="type" class="loc"
							id="type">
							<option value="common"
								${(questionAddForm.type eq 'common')?"selected='selected'":''}>普通</option>
							<option value="education"
								${(questionAddForm.type eq 'education')?"selected='selected'":''}>教育</option>
							<option value="medical"
								${(questionAddForm.type eq 'medical')?"selected='selected'":''}>医疗</option>
						</select>
					</a>
				...
				<li>
					...
					<a class="emng-select" id="pointValue">
						...
						<select onchange="change($(this))" name="pointValue" class="loc"
							id="pointValue">
							<option value="1"
								${(questionAddForm.pointValue eq 1)?"selected='selected'":''}>1分</option>
							<option value="2"
								${(questionAddForm.pointValue eq 2)?"selected='selected'":''}>2分</option>
							<option value="3"
								${(questionAddForm.pointValue eq 3)?"selected='selected'":''}>3分</option>
							<option value="4"
								${(questionAddForm.pointValue eq 4)?"selected='selected'":''}>4分</option>
							<option value="5"
								${(questionAddForm.pointValue eq 5)?"selected='selected'":''}>5分</option>
						</select>
					</a>
				...
				<li>
					...
					<textarea rows="5" cols="120" id="content" name="content">${questionAddForm.content}</textarea>
				...
				<li>
					...
					<textarea rows="1" cols="120" id="answerCorrect" name="answerCorrect">${questionAddForm.answerCorrect}</textarea>
					<label class="error"></label>
				</li>
				<li>
					<span>
					...
					<textarea rows="2" cols="120" id="answerOne" name="answerOne">${questionAddForm.answerOne}</textarea>
					<label class="error"></label>
				</li>
				<li>
					<span>
					...
					<textarea rows="2" cols="120" id="answerTwo" name="answerTwo">${questionAddForm.answerTwo}</textarea>
					<label class="error"></label>
				</li>
				<li>
					<span>
					...
					<textarea rows="2" cols="120" id="answerThree" name="answerThree">${questionAddForm.answerThree}</textarea>
					<label class="error"></label>
				</li>
			</ul>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			(function() {
				...
				$('#addQuestionForm').enterSub($('#go-submit'));
			})();
